...
def load_assets():
    try:
        ...
        st.error("❌ ไม่พบไฟล์โมเดล! กรุณาตรวจสอบไฟล์ใน GitHub")
        return None, None
...
if model:
    ...
    if submit_btn:
        ...
        with res_col1:
            # หน้าปัดแสดงความเสี่ยง (Gauge Chart)
            fig = go.Figure(go.Indicator(
                mode = "gauge+number",
                value = probability * 100,
                number = {'suffix': "%", 'font': {'size': 50}},
                title = {'text': "ความน่าจะเป็นที่จะเป็นกลุ่มเสี่ยง", 'font': {'size': 20}},
                gauge = {
        ...
        with res_col2:
            ...
            st.info("💡 **คำแนะนำ:** ผลลัพธ์นี้เป็นเพียงการประเมินเบื้องต้นจาก AI ควรใช้ร่วมกับการวินิจฉัยโดยแพทย์ผู้เชี่ยวชาญ")
